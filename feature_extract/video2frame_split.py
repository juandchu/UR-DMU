...
def dump_frames(vid_item):
    ...
    out_full_path = osp.join(
        "video_data_frames", vid_name[0], vid_name[1].replace(".mp4", "")
    )
    ...
    for i in range(videolen):
        ...
        if img is not None:
            ...
            break
    print(
        "full_path={} vid_name={} num_frames={} dump_frames done".format(
            full_path, vid_name, videolen
        )
    )
    sys.stdout.flush()
    return True
...
def parse_args():
    ...
    parser.add_argument("--src_dir", default="data/", type=str)
