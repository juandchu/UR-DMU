...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn1 = self.attend(dots)  # global memory (all to all connections) (per bag)
        ...
        )  # Exponential weights for nearby frames(the further away the less improtant)

        out = torch.cat(
            [torch.matmul(attn1, v), torch.matmul(attn2, t)], dim=-1
        )  # (concatenate the ouput and then squash it back to 512D)
